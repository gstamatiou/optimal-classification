...
def newton_energy(u0_flat, W, train_ind, y, p, rate = 0.2, tol=1e-4, max_iter=1e4, verbose = False):
    ...
    project = lambda u:  u - ((1/D.sum()) * (np.diag(D) @ u).sum(axis = 0))
    u_flat = u0_flat
    
    difference = tol + 1
    it = 0
    while it < max_iter and difference > tol:
        u_flat_bar = u_flat - inv(hessian(u_flat, W, train_ind, y, p)) @ jacobian(u_flat, W, train_ind, y, p)
        uu_flat = project(u_flat_bar.reshape((n,k))).flatten()

        it += 1
        difference = norm(uu_flat - u_flat)
        if verbose:
            print(difference)
            print(u_flat)
...
# a = rand(2) * 5
# jac = lambda x: 2 * (x - a)
# hess = lambda x: np.eye(x.shape[0])
# proj = lambda x: x / norm(x)
# sol = newton(rand(2), proj, jac, hess, verbose = True)
# 
# fig, ax = plt.subplots()
# ax.axis('equal')
